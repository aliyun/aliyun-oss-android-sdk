    ...
    public void sign(RequestMessage request) throws ClientException {
        ...
        if (credentialProvider instanceof OSSCustomSignerCredentialProvider) {
            ...
            try {
                ...
                content.put(CONTENT_STRING_TO_SIGN, stringToSign);
                content.put(CONTENT_DATE, getDate());
                content.put(CONTENT_ALGORITHM, OSS4_HMAC_SHA256);
                content.put(CONTENT_PRODUCT, signerParams.getProduct());
                content.put(CONTENT_REGION, signerParams.getRegion());
                JSONObject jsonObj = new JSONObject(content);
                String authorization = customSignerCredentialProvider.signContent(jsonObj.toString());
    ...
    public void presign(RequestMessage request) throws ClientException {
        ...
        if (credentialProvider instanceof OSSCustomSignerCredentialProvider) {
            try {
                ...
                String credentialAndSignature = authorization.split(" ")[1];
                String credential = null;
                String[] credentialAndSignatureStrings = credentialAndSignature.split(",");
                if (credentialAndSignatureStrings.length == 2) {
                    if (credentialAndSignatureStrings[0].contains("Credential=")) {
                        credential = credentialAndSignatureStrings[0].substring(11);
                    }
                    if (credentialAndSignatureStrings[1].contains("Signature=")) {
                        signature = credentialAndSignatureStrings[1].substring(10);
                    }
                }

                if (signature == null || credential == null) {
                    throw new ClientException("Cannot get credential or signature!");
                }
